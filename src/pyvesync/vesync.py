...
from pyvesync.devhelpers import device_debug_helper
...
def object_factory(config: dict, manager: VeSync
                   ) -> Tuple[str, Optional[VeSyncBaseDevice]]:
    """Get device type and instantiate class."""

    def fans(config_module, config, manager):
        fan_cls = fan_mods.fan_modules[config_module]
        fan_obj = getattr(fan_mods, fan_cls)
        return "fans", fan_obj(config, manager)

    def outlets(config_module, config, manager):
        outlet_cls = outlet_mods.outlet_modules[config_module]
        outlet_obj = getattr(outlet_mods, outlet_cls)
        return "outlets", outlet_obj(config, manager)

    def switches(config_module, config, manager):
        switch_cls = switch_mods.switch_modules[config_module]
        switch_obj = getattr(switch_mods, switch_cls)
        return "switches", switch_obj(config, manager)

    def bulbs(config_module, config, manager):
        bulb_cls = bulb_mods.bulb_modules[config_module]
        bulb_obj = getattr(bulb_mods, bulb_cls)
        return "bulbs", bulb_obj(config, manager)

    def kitchen(config_module, config, manager):
        kitchen_cls = kitchen_mods.kitchen_modules[config_module]
        kitchen_obj = getattr(kitchen_mods, kitchen_cls)
        return "kitchen", kitchen_obj(config, manager)

    config_module = config.get("configModule")
    if config_module is None:
        logger.debug("No configModule found in device config - Unknown Device")
        logger.debug("Full Device Config - \n %s", json.dumps(config, indent=4))
        return "unknown", None
    if config_module in fan_mods.fan_modules:
        type_str, dev_obj = fans(config_module, config, manager)
    elif config_module in outlet_mods.outlet_modules:
        type_str, dev_obj = outlets(config_module, config, manager)
    elif config_module in switch_mods.switch_modules:
        type_str, dev_obj = switches(config_module, config, manager)
    elif config_module in bulb_mods.bulb_modules:
        type_str, dev_obj = bulbs(config_module, config, manager)
    elif config_module in kitchen_mods.kitchen_modules:
        type_str, dev_obj = kitchen(config_module, config, manager)
    else:
        logger.debug(
        ...
        if manager.debug is True:
            device_debug_helper(config, manager)
            type_str = "unknown"
            dev_obj = None
...
class VeSync:  # pylint: disable=function-redefined
    ...
    def add_dev_test(self, new_dev: dict) -> bool:
        ...
        new_id = new_dev.get("cid") or new_dev.get("uuid")
        for dev in chain(*self._dev_list.values()):
            if dev.cid == new_id and new_dev.get("subDeviceNo", 0) == dev.sub_device_no:
                return False
        return True
    ...
    def process_devices(self, dev_list: list) -> bool:
        ...
        for dev in devices:
            if not all(k in dev for k in detail_keys):
                ...
                continue
            dev_type = dev.get("configModule")
            try:
                device_str, device_obj = object_factory(dev, self)

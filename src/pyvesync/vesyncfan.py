...
from pyvesync.vsfanspecs import humid_features, air_features
from pyvesync.devhelpers import (Timer,
                                 DeviceConfig,
                                 FANFEATURES,
                                 FANMODES)
...
# Air purifier and humidifer feature dictionary from vsfans.py
fan_features = air_features | humid_features

fan_classes: set = {v["module"] for k, v in fan_features.items()}

fan_modules: dict = DeviceConfig.model_dict(fan_features)
...
class VeSyncAirBypass(VeSyncBaseDevice):
    ...
    def __init__(self, details: Dict[str, list], manager):
        ...
        self.enabled = True
        self.config_dict = DeviceConfig.get_config_dict(self.config_module, fan_features)
        self.features = DeviceConfig.get_features(self.config_module, fan_features)
        self.modes = DeviceConfig.get_modes(self.config_module, fan_features)
        self.levels = self.config_dict.get("levels", [])
        if self.modes == []:
            logger.error(
        ...
        if FANFEATURES.AIR_QUALITY in self.features:
    ...
    def get_timer(self) -> Optional[Timer]:
        ...
        if r.get("code") != 0:
            logger.debug(
                "Error in purifier response, code %s", r.get("code", "unknown")
            )
            return None
        if r.get("result", {}).get("code") != 0:
            logger.debug(
                "Error in purifier result response, code %s",
        ...
        if self.timer is None:
            self.timer = Timer(
                timer_duration=timer.get("duration", 0),
    ...
    def set_timer(self, timer_duration: int) -> bool:
        ...
        if timer_id is not None:
            self.timer = Timer(timer_duration=timer_duration, action="off", id=timer_id)
        else:
            self.timer = Timer(timer_duration=timer_duration, action="off")
        return True
    ...
    def clear_timer(self) -> bool:
        ...
        body["payload"]["data"] = {"id": self.timer.id}
    ...
    def mode_toggle(self, mode: str) -> bool:
        ...
        if mode.lower() not in [a.value for a in self.modes]:
    ...
    def manual_mode(self) -> bool:
        """Set mode to manual."""
        if FANMODES.MANUAL not in self.modes:
    ...
    def sleep_mode(self) -> bool:
        ...
        if FANMODES.SLEEP not in self.modes:
    ...
    def auto_mode(self) -> bool:
        """Set mode to auto."""
        if FANMODES.AUTO not in self.modes:
        ...
        if self.air_quality_feature:
            disp.extend(
                [
                    ...
                    (
                        "Air Quality Value: ",
                        self.details.get("air_quality_value", ""),
                        "ug/m3",
                    ),
                ]
            )
...
class VeSyncVital(VeSyncAirBypass):
    ...
    def build_api_dict(self, method: str) -> Tuple[Dict, Dict]:
        ...
        body["payload"] = {"method": method, "source": "APP", "data": {}}
        return header, body
    ...
    def build_purifier_dict(self, dev_dict: dict) -> None:
        ...
        if isinstance(dev_dict.get("autoPreference"), dict):
            self.details["auto_preference_type"] = dev_dict.get(
                "autoPreference", {}
            ).get("autoPreferenceType", "default")
        else:
    ...
    def toggle_switch(self, toggle: bool) -> bool:
        ...
        if toggle is True:
            ...
            power = 0
        body["payload"]["data"] = {"powerSwitch": power, "switchIdx": 0}
    ...
    def set_child_lock(self, mode: bool) -> bool:
        ...
        if mode:
            ...
            toggle_id = 0
        head, body = self.build_api_dict("setChildLock")
        body["payload"]["data"] = {"childLockSwitch": toggle_id}
    ...
    def set_display(self, mode: bool) -> bool:
        ...
        if mode:
            ...
            mode_id = 0
        head, body = self.build_api_dict("setDisplay")
        body["payload"]["data"] = {"screenSwitch": mode_id}
    ...
    def set_timer(
        self, timer_duration: int, action: str = "off", method: str = "powerSwitch"
    ) -> bool:
    ...
    def set_auto_preference(
        self, preference: str = "default", room_size: int = 600
    ) -> bool:
    ...
    def mode_toggle(self, mode: str) -> bool:
        ...
        body["deviceId"] = self.cid
        body["payload"]["data"] = {"workMode": mode.lower()}
...
class VeSyncHumid200300S(VeSyncBaseDevice):
    ...
    def __init__(self, details, manager):
        ...
        if FANFEATURES.WARM_MIST in self.features:
        ...
        if FANFEATURES.NIGHTLIGHT in self.config_dict.get("features", []):
    ...
    def set_humidity_mode(self, mode: str) -> bool:
        ...
        if mode.lower() not in [a.value for a in self.mist_modes]:
    ...
    def set_auto_mode(self):
        ...
        if "auto" in [a.value for a in self.mist_modes]:
            call_str = "auto"
        elif "humidity" in [a.value for a in self.mist_modes]:
            call_str = "humidity"
        else:
            logger.debug(
...
class VeSyncHumid1000S(VeSyncHumid200300S):
    ...
    def build_humid_dict(self, dev_dict: Dict[str, str]) -> None:
        ...
        self.details["humidity_high"] = bool(
            int(dev_dict.get("targetHumidity", 0)) < int(dev_dict.get("humidity", 0))
        )
    ...
    def set_display(self, mode: bool) -> bool:
        ...
        body["payload"]["data"] = {"screenSwitch": int(mode)}
    ...
    def set_humidity(self, humidity: int) -> bool:
        ...
        body["payload"]["data"] = {"targetHumidity": humidity}
    ...
    def set_automatic_stop(self, mode: bool) -> bool:
        ...
        body["payload"]["data"] = {"autoStopSwitch": int(mode)}
